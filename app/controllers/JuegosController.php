...
class JuegosController extends BaseController {
	...
	public function store()
	{
		...
		$juego->maquina =  Input::get('maquina');
		$juego->agentes_id = Input::get('agentes_id');
		$juego->sorteo = Input::get('sorteo');
		$juego->quiniela = Input::get('quiniela');
		$juego->quiniexpress = Input::get('quiniexpress');
		$juego->juegos = Input::get('juegos');
		$juego->premios = Input::get('premios');
		$juego->pagado = Input::get('pagado');


		if ($juego->quiniela > 0) {
			$juego->quiniela_agencia = $juego->quiniela * 0.05;
			$juego->quiniela_agente = $juego->quiniela * 0.15;
			$juego->quiniela_pagar = $juego->quiniela - $juego->quiniela_agente;
		}

		if ($juego->quiniexpress > 0) {
			$juego->quiniexpress_agencia = $juego->quiniexpress * 0.05;
			$juego->quiniexpress_agente = $juego->quiniexpress * 0.08;
			$juego->quiniexpress_pagar = $juego->quiniexpress - $juego->quiniexpress_agente;
		}


		if ($juego->juegos > 0) {
			$juego->juegos_agencia = $juego->juegos * 0.05;
			$juego->juegos_agente = $juego->juegos * 0.10;
			$juego->juegos_pagar = $juego->juegos - $juego->juegos_agente;
		}

		$juego->total_maquina = $juego->quiniela + $juego->quiniexpress + $juego->juegos;
		$juego->total_pagar = $juego->quiniela_pagar + $juego->quiniexpress_pagar + $juego->juegos_pagar;
		$juego->neto_pagar = $juego->total_pagar - $juego->premios;

		$juego->deuda = $juego->pagado - $juego->neto_pagar;
